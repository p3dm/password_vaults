...
from controller.log import Log_Record as log
...
def validate_credential(data: dict) -> dict:
    ...
    if errors:
        log.log_error(event_type = "validation error", message = "; ".join(errors), object_id = str(data.get("id")))
...
def validate_rule_input(rule: dict) -> dict:
    ...
    match_type = rule.get("match_type", "").strip()
    ...
    if match_type == "window_title_regex" and len(match_value) > MAX_REGEX_LENGTH:
